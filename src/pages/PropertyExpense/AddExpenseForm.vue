                ...
                <div class="row">
                  <div class="col-md-3">
                    ...
                    <select class="form-control" v-model="form.type">
                      <option  v-for="(index, i) in type" :key="i" :value="i">{{ index }}</option>
                    </select>
  ...
  export default {
    ...
    methods: {
        ...
        this.loading=true;
          ...
          .then(response => {
              if(response.status==200 && response.data.documentTypes){
                this.type = response.data.documentTypes
              }
              this.loading=false;
          })
          .catch(error => {
      ...
      saveProfile () {
        ...
        this.loading=true;
        axios.post(process.env.VUE_APP_API_URL+'expense/add', this.form, {
          headers: {
              'Authorization' : 'Bearer '+ localStorage.getItem('token')
          },
          })
          .then(response => {
            ...
            if(response.status == 200 && response.data){
              this.$notifications.notify(
              {
                message: '<span>New record addedd successfully</span>',
                icon: 'nc-icon nc-bell-55',
                horizontalAlign: 'right',
                verticalAlign: 'top',
                type: 'success'
