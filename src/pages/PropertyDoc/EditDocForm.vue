  ...
  export default {
    ...
    methods: {
      ...
      getOwner(id){
          ...
          axios.get(process.env.VUE_APP_API_URL+'document/'+id+'/edit',{
            headers: {
                'Content-Type': 'application/json',
                'Authorization' : 'Bearer '+ localStorage.getItem('token')
            }
          ...
          .then(response => {
              if(response.status==200 && response.data.documentDetails){
                currentObj.form.property_id = response.data.documentDetails.property_id
                currentObj.form.type = response.data.documentDetails.type
                currentObj.form.care_taker_name = response.data.documentDetails.care_taker_name
                currentObj.form.start_date = response.data.documentDetails.start_date
                currentObj.form.end_date = response.data.documentDetails.end_date
                currentObj.form.pdf_name = response.data.documentDetails.name
                currentObj.form.id = response.data.documentDetails.id
              }
              this.loading=false;
          })
          .catch(error => {
      ...
      saveProperty () {
        ...
        this.loading=true;
        const url = process.env.VUE_APP_API_URL+'document/update'
