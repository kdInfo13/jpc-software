  ...
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
          <card>
            <h4 slot="header" class="card-title">Edit Trader</h4>
            <form>
              <div class="row">
                <div class="col-md-4">
...
export default {
  ...
  methods: {
    ...
    getOwner(id) {
      this.loading=true
      axios.get(process.env.VUE_APP_API_URL+'trade-person/'+id+'/edit',{
              headers: {
            ...
            .then(response => {
                if(response.status==200 && response.data.tradePersons){
                  this.form.admin_id = response.data.tradePersons.admin_id
                  this.form.id = response.data.tradePersons.id
                  this.form.name = response.data.tradePersons.name
                  this.form.email = response.data.tradePersons.email
                  this.form.phone_no = response.data.tradePersons.phone_no
                }
                this.loading=false;
            }).catch(error => {
              console.log(error)
    ...
    updateProfile () {
      ...
      this.loading=true;
      axios.post(process.env.VUE_APP_API_URL+'trade-person/update', this.form, {
          headers: {
              'Authorization' : 'Bearer '+ localStorage.getItem('token')
          },
          })
          .then(response => {
            ...
            if(response.status == 200 && response.data){
              this.$notifications.notify(
              {
                message: '<span>Trade person updated successfully.</span>',
                icon: 'nc-icon nc-bell-55',
                horizontalAlign: 'right',
                verticalAlign: 'top',
                type: 'success'
